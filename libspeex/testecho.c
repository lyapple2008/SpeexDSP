...
int main(int argc, char **argv)
{
   ...
   rec_fd = fopen(argv[1], "rb");
   echo_fd = fopen(argv[2], "rb");
   e_fd    = fopen(argv[3], "wb");
   if (!echo_fd || !rec_fd || !e_fd){
	   printf("Fail to open file!!!\n");
	   return -1;
   }

   if (read_header(&echo_header, echo_fd) != 0){
	   printf("Fail to parse wav file!\n");
	   return -1;
   }
   if (read_header(&ref_header, rec_fd) != 0){
	   printf("Fail to parse wav file!\n");
	   return -1;
   }
   write_header(&echo_header, e_fd);
   sampleRate = echo_header.format.sample_per_sec;
